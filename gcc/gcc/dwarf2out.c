...
static GTY(()) section *debug_control_flow;
			  ...
			  const char *file ATTRIBUTE_UNUSED)
{
  ...
  if (flag_dwarf_control_flow)
    dwarf2out_bb_debug (ENTRY_BLOCK_PTR_FOR_FN (cfun));
...
dw_val_equal_p (dw_val_node *a, dw_val_node *b)
{
  ...
  switch (a->val_class)
    ...
    case dw_val_class_secptr:
      return a->v.val_secptr.secptr == b->v.val_secptr.secptr;
...
static void dwarf2out_begin_epilogue (unsigned int, const char *);
...
/* Control flow related structs and variables */
typedef struct control_flow_entry {
  struct control_flow_entry*  next;
  unsigned int                index;
  int                         bb_index;
  unsigned int                num_arcs;
  struct control_flow_entry** vec_arc;
  bool                        seen;
  tree                        fn_decl;
} control_flow_entry;

typedef struct control_flow_entry_fn {
   tree                     fn_decl;
   struct control_flow_entry*    fn_entry_block;
   struct control_flow_entry_fn* prev;
} control_flow_entry_fn;

/* pointer to the last allocated bb entry in chain */
typedef struct control_flow_entry_list
{
  control_flow_entry* first; /* first chain entry */
  control_flow_entry* last;  /* last chain entry  */
  unsigned int   index; /* number of entries */
} control_flow_entry_list;

/* list of all entries in this compilation unit */
static control_flow_entry_list control_flow_entries = {NULL,NULL,0};

/* index of last allocated control_flow_entry */
static control_flow_entry* control_flow_entry_active = NULL;

static struct control_flow_entry_fn* control_flow_entry_fn_last = NULL;
...
static control_flow_entry* find_control_flow_entry (basic_block bb);
static control_flow_entry_fn* find_control_flow_entry_fn (tree fun);
static control_flow_entry* alloc_control_flow_entry (void);
static control_flow_entry* alloc_control_flow_entry_bb (basic_block bb);
static control_flow_entry_fn* alloc_control_flow_entry_fn (tree fun, control_flow_entry* entry);
static void print_control_flow_end_function (void);
static void print_control_flow_entries (void);
static void record_control_flow_entries (tree fun);
static void print_debug_control_flow (void);
static void init_debug_control_flow (void);
static inline section* AT_secptr (dw_attr_ref);
static inline const char* AT_secptr_lbl (dw_attr_ref);
static inline void add_AT_secptr (dw_die_ref, enum dwarf_attribute,
                                  const char*, section*);
...
#endif
#ifndef DEBUG_CONTROL_FLOW_SECTION
#define DEBUG_CONTROL_FLOW_SECTION ".debug_control_flow"
#endif
...
#endif
#ifndef BASIC_BLOCK_BEGIN_LABEL
#define BASIC_BLOCK_BEGIN_LABEL	"Lcontrol_flow_BBB"
#endif
#ifndef BASIC_BLOCK_END_LABEL
#define BASIC_BLOCK_END_LABEL	"Lcontrol_flow_BBE"
#endif
#ifndef DWARF_CONTROL_FLOW_LABEL
#define DWARF_CONTROL_FLOW_LABEL     "Ldebug_control_flow"
#endif
#ifndef DWARF_CONTROL_FLOW_ENTRY_LABEL
#define DWARF_CONTROL_FLOW_ENTRY_LABEL     "Lcontrol_flow_entry"
#endif
...
output_die (dw_die_ref die)
{
  ...
  FOR_EACH_VEC_SAFE_ELT (die->die_attr, ix, a)
	...
	case dw_val_class_secptr:
	  dw2_asm_output_offset (DWARF_OFFSET_SIZE, AT_secptr_lbl (a),
				 AT_secptr (a), "%s", name);
	  break;
...
gen_subprogram_die (tree decl, dw_die_ref context_die)
{
  ...
  control_flow_entry_fn* entry = find_control_flow_entry_fn (decl);
  if (entry)
  {
    char label[MAX_ARTIFICIAL_LABEL_BYTES];

    ASM_GENERATE_INTERNAL_LABEL (label, DWARF_CONTROL_FLOW_ENTRY_LABEL,
                                 entry->fn_entry_block->index);
    add_AT_secptr (subr_die, DW_AT_control_flow, label, debug_control_flow);
  }
}
...
{
  /* remember the cfg for current function */
  if (flag_dwarf_control_flow)
    record_control_flow_entries (fun);
...
dwarf2out_end_function (unsigned int)
{
  ...
  maybe_at_text_label_p = false;
  print_control_flow_end_function ();
}
...
dwarf2out_finish (const char *filename)
{
  ...
  /* output the collected cfg info */
  if (flag_dwarf_control_flow)
    print_debug_control_flow ();
...
/* Add a section offset attribute value to a DIE, an offset into an
   arbitrary section.  */

static inline void
add_AT_secptr (dw_die_ref die, enum dwarf_attribute attr_kind,
		const char *label, section *secptr)
{
  dw_attr_node attr;

  attr.dw_attr = attr_kind;
  attr.dw_attr_val.val_class = dw_val_class_secptr;
  attr.dw_attr_val.v.val_secptr.lbl = xstrdup (label);
  attr.dw_attr_val.v.val_secptr.secptr = secptr;
  add_dwarf_attr (die, &attr);
}

static inline const char *
AT_secptr_lbl (dw_attr_ref a)
{
  gcc_assert (a && (AT_class (a) == dw_val_class_secptr));
  return a->dw_attr_val.v.val_secptr.lbl;
}

static inline section *
AT_secptr (dw_attr_ref a)
{
  gcc_assert (a && (AT_class (a) == dw_val_class_secptr));
  return a->dw_attr_val.v.val_secptr.secptr;
}

void
dwarf2out_bb_debug (basic_block bb)
{
  control_flow_entry* entry;

  if (control_flow_entry_active) {
    ASM_OUTPUT_DEBUG_LABEL (asm_out_file, BASIC_BLOCK_END_LABEL,
      control_flow_entry_active->index);
    control_flow_entry_active = NULL;
  }

  if (!bb)
    return;

  entry = find_control_flow_entry (bb);

  gcc_assert (entry);

  if (entry->seen) /* yes this happens: last minute bb duplication. */
  {
    entry = alloc_control_flow_entry_bb (bb);
  }

  gcc_assert (entry);

  ASM_OUTPUT_DEBUG_LABEL (asm_out_file, BASIC_BLOCK_BEGIN_LABEL, entry->index);
  entry->seen = true;
  control_flow_entry_active = entry;
}

/* find entry in chain by basic block reference */
static control_flow_entry*
find_control_flow_entry (basic_block bb)
{
  control_flow_entry* entry;
  ...
  for (entry = control_flow_entries.first; entry != NULL; entry = entry->next)
  {
    if ((entry->fn_decl == current_function_decl) && (bb->index == entry->bb_index))
      return entry;
  }
  return NULL;
}

/* find entry in chain by tree reference */
static control_flow_entry_fn*
find_control_flow_entry_fn (tree fun)
{
  control_flow_entry_fn* entry;
  for (entry = control_flow_entry_fn_last; entry != NULL; entry = entry->prev)
  {
    if (fun == entry->fn_decl)
      return entry;
  }
  return NULL;
}

/* Create and append a new entry to the entry chain */
static control_flow_entry*
alloc_control_flow_entry (void)
{
  control_flow_entry* entry = (control_flow_entry*) xcalloc (1,
    sizeof (control_flow_entry));

  gcc_assert(entry);

  memset (entry, 0, sizeof (control_flow_entry));
  entry->index = ++control_flow_entries.index;
  entry->seen = false;

  entry->next= NULL;
  if (!control_flow_entries.first)
    control_flow_entries.first = entry;

  if (control_flow_entries.last)
    control_flow_entries.last->next = entry;

  control_flow_entries.last = entry;

  return entry;
}

static control_flow_entry*
alloc_control_flow_entry_bb (basic_block bb)
{
  control_flow_entry* entry = alloc_control_flow_entry ();

  gcc_assert(entry);

  entry->fn_decl = current_function_decl;
  entry->bb_index = bb->index;
  entry->num_arcs = EDGE_COUNT (bb->succs);
  if (entry->num_arcs)
  {
    edge_iterator ei;
    edge e;
    ...
    entry->vec_arc = (control_flow_entry**) xcalloc (entry->num_arcs,
      sizeof (control_flow_entry*));
    FOR_EACH_EDGE(e, ei, bb->succs)
    {
      control_flow_entry* succ;
      succ = find_control_flow_entry (e->dest);
      if (!succ)
        succ = alloc_control_flow_entry_bb (e->dest);

      entry->vec_arc[idx++] = succ;
    }
  }
  return entry;
}
...
static control_flow_entry_fn*
alloc_control_flow_entry_fn (tree fun, control_flow_entry* entry_block)
{
  control_flow_entry_fn* entry = (control_flow_entry_fn*) xcalloc (1,
    sizeof (control_flow_entry_fn));

  gcc_assert(entry);

  entry->fn_decl = fun;
  entry->fn_entry_block = entry_block;
  entry->prev = control_flow_entry_fn_last;
  control_flow_entry_fn_last = entry;

  return entry;
}

/* Output all entries at once to the output file */
static void
print_control_flow_entries (void)
{
  control_flow_entry* entry;
  ...
  for (entry = control_flow_entries.first; entry != NULL; entry = entry->next)
  {
    ...
    char label_begin[MAX_ARTIFICIAL_LABEL_BYTES];
    char label_end[MAX_ARTIFICIAL_LABEL_BYTES];

    unsigned int iter;
    unsigned int num_seen_successors = 0;

    /* print bb label  */
    ASM_OUTPUT_DEBUG_LABEL (asm_out_file, DWARF_CONTROL_FLOW_ENTRY_LABEL,
                            entry->index);

    ASM_GENERATE_INTERNAL_LABEL (label_begin, BASIC_BLOCK_BEGIN_LABEL,
                                 entry->index);
    ASM_GENERATE_INTERNAL_LABEL (label_end, BASIC_BLOCK_END_LABEL,
                                 entry->index);

    /* print reference to text section */
    dw2_asm_output_addr (DWARF2_ADDR_SIZE, label_begin,
                           "basic block %d", entry->bb_index);

    /* print size in text section as uleb128 difference of two labels */
    dw2_asm_output_delta_uleb128 (label_end, label_begin, "size of basic block");

    /* If we didn't see a block in output, this means there is no real arc to it,
     * so don't count them.
     */
    for (iter = 0; iter < entry->num_arcs; ++iter)
      if (entry->vec_arc[iter]->seen)
        ++num_seen_successors;

    /* print number of successors */
    dw2_asm_output_data_uleb128 (num_seen_successors, "number of successors");

    /* iterate over successors (if any) */
    if (!num_seen_successors)
      continue;

    for (iter = 0; iter < entry->num_arcs; ++iter)
    {
      control_flow_entry* succ = entry->vec_arc[iter];

      if (!succ->seen)
        continue;

      /* print reference to successor */
      ASM_GENERATE_INTERNAL_LABEL (label_begin,
                                   DWARF_CONTROL_FLOW_ENTRY_LABEL,
                                   succ->index);
      dw2_asm_output_offset (DWARF_OFFSET_SIZE, label_begin,
                             debug_control_flow,
                             "arc to basic block %d", succ->bb_index);
    }
  }
}
...
/* Traverse the call graph and record the structure
 * TODO: Remember the pointer to the entry block of the function,
 *        because it is needed in the DW_TAG_subprogram.
 */
static void
record_control_flow_entries (tree fun)
{
  basic_block bb;
  struct control_flow_entry* first;

  init_debug_control_flow ();

  first = alloc_control_flow_entry_bb (ENTRY_BLOCK_PTR_FOR_FN (cfun));
  first->fn_decl = fun;
  alloc_control_flow_entry_fn (fun, first);

  /* iterate over current function definition */
  FOR_ALL_BB_FN (bb, cfun)
    if (NULL == find_control_flow_entry (bb))
      alloc_control_flow_entry_bb (bb);
}

static void
print_debug_control_flow (void)
{
  if (NULL == control_flow_entries.first) return;

  init_debug_control_flow ();

  /* print section */
  switch_to_section (debug_control_flow);

  print_control_flow_entries ();
}

static void
init_debug_control_flow (void)
{
  if (NULL != debug_control_flow) return;
  debug_control_flow = get_section (DEBUG_CONTROL_FLOW_SECTION, SECTION_DEBUG,
                                    NULL);
}
...
{
  if (flag_dwarf_control_flow)
    dwarf2out_bb_debug (EXIT_BLOCK_PTR_FOR_FN (cfun));
}
