...
decl_attributes (tree *node, tree attributes, int flags)
{
  ...
  /* HDP-343 (4.6), HDP-859 (4.9): handle -finline-is-always-inline and
     -finline-is-externally-visible options.  */
  if (TREE_CODE (*node) == FUNCTION_DECL
      && DECL_DECLARED_INLINE_P (*node))
    {
      if (htc_opt_inline_is_always_inline
          && lookup_attribute ("always_inline", attributes) == NULL)
        attributes = tree_cons (get_identifier ("always_inline"),
                    NULL, attributes);

      if (htc_opt_inline_is_externally_visible
          && lookup_attribute ("externally_visible", attributes) == NULL)
        attributes = tree_cons (get_identifier ("externally_visible"),
	                NULL, attributes);
    }
