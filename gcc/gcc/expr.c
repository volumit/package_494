		...
		if (TYPE_PRECISION (type) < BITS_PER_WORD)
		  {
		    ...
		    /* Make sure the bits beyond the original bitsize are zero
		       ...
		       later constructor elements, cf. HDP-1515 / PR71700. */
		    HOST_WIDE_INT bitsize_mask
		      = (HOST_WIDE_INT_C(2) << (bitsize - 1)) - 1;
		    value
		      = fold_build2 (BIT_AND_EXPR, type, value,
				     build_int_cst (type, bitsize_mask));
		  }
