...
;; of an OR or XOR.
;; In VLE mode also SCI8 operands are valid (GCC-249)
  ...
  (match_code "const_int")
{
  ...
  return (((opl & ~ (unsigned HOST_WIDE_INT) 0xffff) == 0
	  ...
	  || ((TARGET_VLE) && ppcvle_valid_sci8((unsigned HOST_WIDE_INT) (opl))));
})
  ...
  (match_code "parallel")
{
  ...
  /* HDP-132 the last entry can be a USE rtx */
  if (GET_CODE (XVECEXP (op, 0, count - 1)) == USE)
    count--;
...
;; Return 1 if op is a constant integer satisfy constraint "wD" or "I"
;; or non-special register.
(define_predicate "reg_or_short_vle_operand"
  (if_then_else (match_code "const_int")
    (if_then_else (match_test "(TARGET_VLE)")
        (match_test "satisfies_constraint_wD (op)")
        (match_operand 0 "short_cint_operand"))
    (match_operand 0 "gpc_reg_operand")))

;; Return 1 if op is an unsigned constant integer satisfy constraint
;; "wD" or "I" or non-special register.
(define_predicate "reg_or_u_short_vle_operand"
  (if_then_else (match_code "const_int")
    (if_then_else (match_test "(TARGET_VLE)")
        (match_test "satisfies_constraint_wD (op)")
        (match_operand 0 "u_short_cint_operand"))
...
;;
;; return 1 if op is a simple ref in code pic
(define_predicate "pic_no_const_operand"
  (match_code "symbol_ref,label_ref")
 {
    if (TARGET_CODE_PIC)
    {
        if (GET_CODE(op) == SYMBOL_REF)
            return SYMBOL_FLAG_CO_PIC_P(op);
