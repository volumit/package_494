...
/* HTC specific additions */
static v850_section_t *v850_lookup_section (const char *name);
static unsigned int v850_section_flags_from_string (const char *s_flags);
static void v850_section_flags_to_string (unsigned int flags, char *s_flags);
static bool v850_section_check_arguments (const v850_section_t *section, unsigned int req_flags, unsigned int req_align, location_t location);
static void v850_set_decl_from_section (tree decl, v850_section_t *secp);

v850_section_t *v850_insert_section (const char *name, const char *s_flags, unsigned int align, location_t location);


/* A linked list to store the sections */
static GTY(()) v850_section_t *v850_sections;

extern GTY(()) v850_section_t *v850_pragma_section_code;
v850_section_t *v850_pragma_section_code;

extern GTY(()) v850_section_t *v850_pragma_section_data;
v850_section_t *v850_pragma_section_data;
...
static v850_section_t *
v850_lookup_section (const char *name)
{
  v850_section_t *secp = NULL;

  for (secp = v850_sections; secp; secp = secp->next)
    if (!strcmp (name, secp->name))
      break;

  return secp;
}

/* Covert the input section flags to an unsigned integer. */
static unsigned int
v850_section_flags_from_string (const char *s_flags)
{
...
/* Convert the input section flags to a character representation (s_flags). */
static void
v850_section_flags_to_string (unsigned int flags, char *s_flags)
{
...
static void
v850_set_decl_from_section (tree decl, v850_section_t *secp)
{
  /* The decl may have already been given a section attribute
     from a previous declaration.  Ensure they match.  */

  if (DECL_SECTION_NAME (decl) != NULL_TREE
      && 0 != strcmp (secp->name, TREE_STRING_POINTER (DECL_SECTION_NAME (decl))))
    {
      error ("section of %q+D conflicts with previous declaration", decl);

      return;
    }

  DECL_SECTION_NAME(decl) = build_string (strlen(secp->name), secp->name);

  if (secp->align)
    {
      DECL_ATTRIBUTES(decl) = tree_cons (get_identifier("aligned"),
                                         tree_cons (NULL_TREE, size_int (secp->align), NULL_TREE),
                                         DECL_ATTRIBUTES (decl));
      DECL_ALIGN(decl) = secp->align * BITS_PER_UNIT;
...
static tree
v850_handle_decl_attribute (tree *node, tree name,
...
v850_section_t*
v850_insert_section (const char *name, const char *s_flags, unsigned int align, location_t location)
{
  unsigned int flags;
  v850_section_t *secp = v850_lookup_section (name);

  /* TODO?: perhaps validate section name? */

  /* Get the flags from the arguments if possible,
     otherwise use the existing section flags if there is any */
  flags = s_flags ? v850_section_flags_from_string (s_flags) : (secp ? secp->flags : SECTION_WRITE);
  ...
  if (align == 0)
    ...
    {
      if (align < 2 && (flags & SECTION_CODE))
        {
          error ("minimum alignment for code section is 2");
  ...
  if (secp)
    {
      /* Check if the given section infos are the same as the existing one */
      if (v850_section_check_arguments (secp, flags, align, location))
        return secp;
      else
        return NULL;    /* Section check returned mismatch */
    }

  /* Create the new section */
  secp = (v850_section_t*) ggc_alloc_cleared_atomic (sizeof (v850_section_t));
  ...
  secp->name = ggc_strdup (name);
  secp->flags = flags;
  secp->align = align;
  secp->next = v850_sections; /* Add section to the section list */
  secp->location = location;

  v850_sections = secp; /* Set the section as the first element in the section list */

  return secp;
}

/* Check if the given section conforms to the given flags and alignment */
static bool
v850_section_check_arguments (const v850_section_t *secp, unsigned int req_flags,
                             unsigned int req_align, location_t location)
{
  const char *s_here = (secp->location == BUILTINS_LOCATION) ? "built-in " : "here";
  location_t loc = (secp->location == BUILTINS_LOCATION) ? location : secp->location;

  if (secp->flags != req_flags)
    {
      char old_flags[20], new_flags[20];

      v850_section_flags_to_string (req_flags, new_flags);
      v850_section_flags_to_string (secp->flags, old_flags);

      error ("section %qs redefined with different flags %s", secp->name, new_flags);
      inform (loc, "section %qs defined %s with flags %s", secp->name, s_here, old_flags);
    }

  if (secp->align != req_align)
    {
      error ("section %qs redefined with different alignment %d", secp->name, req_align);

      if (secp->align)
        inform (loc, "section %qs defined %s with alignment %d", secp->name, s_here, secp->align);
      else
        inform (loc, "section %qs defined %s with no aligment", secp->name, s_here);
    }

  if (secp->flags != req_flags || secp->align != req_align)
    return false;

  return true;
}
/* End of HighTec extras */
...
v850_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  /* Warn if a variable with non-zero initializer is put into
     a .bss-like section. */
  ...
  if (first
    ...
    {
      const char *sec_name = TREE_STRING_POINTER (DECL_SECTION_NAME (decl));
      v850_section_t *v850_sec = v850_lookup_section (sec_name);

      if (v850_sec
          && (v850_sec->flags & SECTION_BSS))
        {
          if (warning (0, "variable %q+D with non-zero initializer is put "
                       "into @nobits section %qs with section flag %<B%>",
                       decl, sec_name))
            {
              char flagc[20];
              const char *s_here = (v850_sec->location == BUILTINS_LOCATION
                                    ? "built-in" : "here");

              v850_section_flags_to_string (v850_sec->flags, flagc);

              inform (v850_sec->location,
                      "section %qs defined %s with flags %qs",
                      sec_name, s_here, flagc);
            }
        }
    }
}
...
{
  if (v850_used_pragma == GHS_PRAGMA)
    {
    if (data_area_stack
   ...
   else
    {
      ...
      if (TREE_CODE (decl) == FUNCTION_DECL)
        secp = v850_pragma_section_code;
      else
        secp = v850_pragma_section_data;

      if (secp == NULL)
        return;

      if ((current_function_decl == NULL_TREE    /* global_scope */
          || (TREE_CODE (decl) == VAR_DECL && TREE_STATIC (decl))) /* local static */
          && (TREE_CODE (decl) == VAR_DECL || TREE_CODE (decl) == CONST_DECL || TREE_CODE (decl) == FUNCTION_DECL)
          && DECL_SECTION_NAME (decl) == NULL)
        {
          if (secp == v850_pragma_section_data
              && htc_opt_pragma_data_sections)
            {
              v850_section_t *v850sec;
              const char *name;
              char s_flags[20];

              name = ACONCAT ((secp->name, ".",
                               IDENTIFIER_POINTER (DECL_NAME (decl)), NULL));

              v850_section_flags_to_string (secp->flags, s_flags);

              v850sec = v850_insert_section (name, s_flags, secp->align,
                                             DECL_SOURCE_LOCATION (decl));

              v850_set_decl_from_section (decl, v850sec);

            }
          else
            {
              /* FUTURE: Add the pragma data section handling here */
              v850_set_decl_from_section (decl, secp);
            }
        }
    }
}
...
static unsigned int
v850_section_type_flags (tree decl, const char *name, int reloc)
{
...
static bool
v850_libc_has_function (enum function_class fn_class)
{
#if DEFAULT_LIBC == LIBC_DINKUM
  return true;
#else
...
#define TARGET_SECTION_TYPE_FLAGS v850_section_type_flags
...
#define TARGET_LIBC_HAS_FUNCTION v850_libc_has_function
